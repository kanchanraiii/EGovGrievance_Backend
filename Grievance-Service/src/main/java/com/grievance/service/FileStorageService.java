...
public class FileStorageService {
    ...
    public FileStorageService(ReactiveGridFsTemplate gridFsTemplate, GrievanceRepository grievanceRepository) {
    ...
    public Mono<String> upload(FilePart file, String grievanceId, String uploadedBy) {
        ...
        return grievanceRepository.findById(grievanceId)
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("Grievance not found")))
                .then(gridFsTemplate.store(validatedContent, file.filename(), metadata))
    ...
    public Flux<FileMetadata> listByGrievance(String grievanceId) {
        ...
        return gridFsTemplate.find(Query.query(Criteria.where("metadata.grievanceId").is(grievanceId)))
                .map(file -> {
                    FileMetadata meta = new FileMetadata();
                    meta.setId(file.getObjectId().toHexString());
                    meta.setFileName(file.getFilename());
                    meta.setContentType(file.getMetadata().getString("contentType"));
                    meta.setGrievanceId(grievanceId);
                    meta.setUploadedBy(file.getMetadata().getString("uploadedBy"));
                    return meta;
                });
    }

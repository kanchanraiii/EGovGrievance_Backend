...
class WebClientConfigTest {
    ...
    @Test
    void propagatesAuthorizationHeaderWhenJwtPresent() {
        ...
        Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").claim("role", "CITIZEN").build();
    ...
    @Test
    void leavesRequestUnchangedWithoutContext() {
        AtomicReference<String> capturedAuth = new AtomicReference<>();
        ExchangeFunction exchangeFunction = request -> {
            capturedAuth.set(request.headers().getFirst(HttpHeaders.AUTHORIZATION));
            return Mono.just(ClientResponse.create(HttpStatus.OK).build());

...
public class DepartmentCatalog {
    ...
    private final AtomicReference<Set<String>> departmentIds;
    ...
    public DepartmentCatalog(
        ...
        this.departmentIds = new AtomicReference<>(loadDepartmentIds(this.departmentsFilePath, this.resourceLoader, this.objectMapper));
    }
    ...
    public boolean isValid(String departmentId) {
        ...
        Set<String> ids = departmentIds.get();
        if (ids.isEmpty()) {
            return false;
        }
        return ids.contains(departmentId.trim().toUpperCase(Locale.ROOT));
    }

    public synchronized void refresh() {
        this.departmentIds.set(loadDepartmentIds(this.departmentsFilePath, this.resourceLoader, this.objectMapper));
    }
    ...
    private Set<String> copyAsImmutable(Set<String> ids) {
        if (ids.isEmpty()) {
            return Set.of();
        }
